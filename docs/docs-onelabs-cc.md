![logo](https://docs.onelabs.cc/img/logo.svg)

Explore

![](https://docs.onelabs.cc/img/indicator.svg)
...
![Language](https://docs.onelabs.cc/img/language.svg)
...
If you used the commands in the [Install using Homebrew](https://docs.onelabs.cc/DevelopmentDocument#install-homebrew) section, you do not need to install anything else.
...
The OneChain Testnet and Devnet networks consist of several validator nodes to validate each network's transactions. Clients send transactions and read requests via this endpoint: `https://rpc-<NETWORK>.onelabs.cc:443` using [JSON-RPC](https://docs.onelabs.cc/RPC).

You can [request test OCT tokens](https://docs.onelabs.cc/DevelopmentDocument#request-test-tokens) through the OneChain [devnet-faucet](https://faucet-devnet.onelabs.cc/) and [testnet-faucet](https://faucet-testnet.onelabs.cc/), depending on which network you use. The coins on these networks have no financial value. There is no faucet service for Mainnet.
...
If you receive the `one client` help output in the console, you already have a client.yaml file. See [Connect to a custom RPC endpoint](https://docs.onelabs.cc/DevelopmentDocument#connect-to-a-custom-rpc-endpoint) to add a new environment alias or to switch the currently active network.
...
For more information, see the [OneChain Client CLI](https://docs.onelabs.cc/references/cli/client.mdx) documentation.

To see all the generated addresses in the local wallet on your machine, run `one keytool list`. For more information about the keytool options, see the [OneChain Keytool CLI](https://docs.onelabs.cc/references/cli/keytool.mdx) documentation.
...
For more information on the types of objects that OneChain supports, see [Object Ownership](https://docs.onelabs.cc/concepts/object-ownership.mdx).
...
The OneChain RPC provides a [queryEvents](https://docs.onelabs.cc/sui-api-ref#suix_queryEvents) method to query on-chain packages and return available events. As an example, the following `curl` command queries the Deepbook package on Mainnet for a specific type of event:
...
- Incorporate a [custom indexer](https://docs.onelabs.cc/advanced/custom-indexer.mdx) to take advantage of OneChain's micro-data ingestion framework.
...
To monitor events, you need a database to store checkpoint data. The [Trustless Swap](https://docs.onelabs.cc/app-examples/trustless-swap.mdx) example uses a Prisma database to store checkpoint data from the OneChain network. The database is populated from polling the network to retrieve emitted events.

[event-indexer.ts](https://github.com/one-chain-labs/onechain/blob/main/examples/trading/api/indexer/event-indexer.ts) from Trustless Swap
...
OneChain uses a SuiKeyPair to produce the signature, which commits to the Blake2b hash digest of the intent message (intent \|\| bcs bytes of tx\_data). The signature schemes currently supported are Ed25519 Pure, ECDSA Secp256k1, ECDSA Secp256r1, Multisig, and zkLogin.
...
This guide explores creating a programmable transaction block (PTB) on OneChain using the TypeScript SDK. For an overview of what a PTB is, see [Programmable Transaction Blocks](https://docs.onelabs.cc/concepts/transactions/prog-txn-blocks.mdx) in the Concepts section. If you don't already have the OneChain TypeScript SDK, follow the [install instructions](https://sdk.mystenlabs.com/typescript/install) on the OneChain TypeScript SDK site.
...
Gas smashing is an important feature - and a key concept to understand - to have for the optimal management of coins. See [Gas Smashing](https://docs.onelabs.cc/concepts/transactions/gas-smashing.mdx) for more details.
...
![/img/x.svg](https://docs.onelabs.cc/img/x.svg)

![/img/telegram.svg](https://docs.onelabs.cc/img/telegram.svg)

![/img/Discord.svg](https://docs.onelabs.cc/img/Discord.svg)

![/img/medium.svg](https://docs.onelabs.cc/img/medium.svg)

![/img/linkedin.svg](https://docs.onelabs.cc/img/linkedin.svg)

![/img/facebook.svg](https://docs.onelabs.cc/img/facebook.svg)

![/img/instagram.svg](https://docs.onelabs.cc/img/instagram.svg)

![/img/youtube.svg](https://docs.onelabs.cc/img/youtube.svg)

![/img/tiktok.svg](https://docs.onelabs.cc/img/tiktok.svg)

![/img/wechat.svg](https://docs.onelabs.cc/img/wechat.svg)

©️ © One Chain Labs Limited. All rights reserved.